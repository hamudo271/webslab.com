...
```
src/
  app/         라우트/페이지 (11 page + 6 route: sitemap 4종, robots, llms.txt, api/contact)
  components/  UI. layout/ · home/ · content/ · common/ · seo/ · analytics/ · contact/ · portfolio/
  lib/         순수 유틸 (테스트 대상). jsonld, metadata, seo-policy, validators, cn, xml
...
- **환경변수(하드코딩 금지)**: `RESEND_API_KEY`(문의 메일), `CONTACT_FROM_EMAIL`·`CONTACT_TO_EMAIL`, `CONTACT_DRY_RUN=true`(메일 안 보내고 로그만), `NEXT_PUBLIC_SITE_URL`, analytics 키들. Railway 대시보드에서 관리.
- **검증**: 변경 후 `npx tsc --noEmit` + `CI=true npx vitest run`(현재 28 테스트). 프리뷰는 헤드리스 `innerHeight:0`이라 스크롤 기반 상태 검증이 불안정 — 내부 페이지(`/about`)의 solid 헤더로 대체 확인.
